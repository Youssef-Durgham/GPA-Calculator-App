...
import { SearchCallbacks } from '../components/SearchCallbacks';
...
const SearchScreen = ({ route, navigation }) => {
  ...
  const handleSelectLocation = (item) => {
    ...
    if (SearchCallbacks.onSelect && typeof SearchCallbacks.onSelect === 'function') {
      SearchCallbacks.onSelect(item);
      // Optionally, remove it after using it:
      delete SearchCallbacks.onSelect;
    }
    navigation.goBack();
  };
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#000B4D' },
  searchContainer: { backgroundColor: 'rgba(255,255,255,0.05)' },
  ...
  resultText: { color: '#fff', fontSize: 16, marginLeft: 12 },
});
